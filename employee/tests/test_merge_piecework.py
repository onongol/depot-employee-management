...
def test_merge_piecework_keys_rows_by_wagon_when_grouping_by_wagon():
    ...
    assert salary_data[(1, 2026, 3, "12")]["total_piecework_amount"] == Decimal("300.00")
    assert salary_data[(1, 2026, 3, "45")]["total_piecework_amount"] == Decimal("100.00")
...
def test_merge_piecework_ignores_wagon_and_defaults_missing_totals_when_not_grouping():
    ...
    salary_data = defaultdict(dict)
    piecework_groups = [{"employee": 1, "work_year": 2026, "work_month": 3, "wagon_number": "12"}]

    merge_piecework(salary_data, piecework_groups=piecework_groups, group_by_wagon=False)
