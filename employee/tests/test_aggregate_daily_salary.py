...
def test_aggregate_daily_salary_sums_by_employee_month_year_with_wagon_forced_none():
    ...
    DailySalaryFactory(employee=employee, hours_per_day=10, salary_date=date(2026, 3, 5))
    DailySalaryFactory(employee=employee, hours_per_day=5, salary_date=date(2026, 3, 20))
    # different month -> excluded by the month filter
    DailySalaryFactory(employee=employee, hours_per_day=24, salary_date=date(2026, 4, 1))
    DailySalaryFactory(employee=other_employee, hours_per_day=4, salary_date=date(2026, 3, 1))
    # not in the `employees` list passed below -> excluded by employee__in
    DailySalaryFactory(employee=excluded_employee, hours_per_day=24, salary_date=date(2026, 3, 1))
    ...
    assert salary_data[(employee.pk, 2026, 3, None)]["total_salary_day"] == Decimal("1500.00")
    assert salary_data[(other_employee.pk, 2026, 3, None)]["total_salary_day"] == Decimal("200.00")
