...
from employee.mixins.context_mixins import DailyPayContextMixin
...
from employee.forms import DailyPayForm, UpdateDailyPayForm
from employee.utils.select_department import get_selected_department
from employee.utils.filters import filter_daily_pays
...
class DailyPayUpdateView(LoginRequiredMixin, OnlyAdminMixin, DailyPayContextMixin, UpdateView):
    login_url = 'login'
    form_class = UpdateDailyPayForm
    template_name = "daily_pay/daily_pay_update.html"


class DailyPayDeleteView(LoginRequiredMixin, OnlyAdminMixin, DailyPayContextMixin, DeleteProtectionMixin, DeleteView):
    login_url = 'login'
    template_name = "daily_pay/daily_pay_confirm_delete.html"
...
def daily_pay_create(request):
    """View to create daily pay records for multiple employees, filtered by department."""
            ...
            try:
                ...
                with transaction.atomic():
                    for emp_id in selected_ids:
                        # Check for duplicate daily pay record for the same employee and date
                        exists = DailyPay.objects.filter(
                            employee_id=emp_id,
                        ...
                        if exists:
                            ...
                            errors.append(
                                f"Daily pay record for Employee: {emp_id}/{emp.name} on {salary_date} already exists!"
                            )
                        else:
                            # Create new DailyPay record
                            DailyPay.objects.create(
                                employee_id=emp_id,
                                salary_date=salary_date,
                                hours_per_day=hours_per_day,
                            )
            except Exception as e:
                errors.append(f"Error creating daily pay records: {str(e)}")
        # If no errors, redirect to the list page for the selected department
        if not errors:
            return redirect(f"{reverse('daily_pay_list')}?department={department}")

    return render(
        request,
        'daily_pay/daily_pay_create.html',
        {
            'form': DailyPayForm(),
            ...
            'cancel_url': reverse('daily_pay_list'),
...
def daily_pay_list(request):
    ...
    if request.user.groups.filter(name='Employees').exists():
        daily_salaries = DailyPay.objects.filter(employee__user=request.user, employee__department=department, employee__is_active=True)
    else:
        # If not an employee, show all daily salaries in the department 
        daily_salaries = DailyPay.objects.filter(employee__department=department, employee__is_active=True)
    ...
    # Apply filters to the daily pays queryset using reusable filter functions
    daily_salaries = filter_daily_pays(
        daily_salaries, 
