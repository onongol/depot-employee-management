...
from employee.views.daily_salary.daily_salary_delete_bulk.bulk_queryset_split import (
    get_deletable_and_blocked_qs,
)
from employee.views.daily_salary.daily_salary_delete_bulk.messages.block_bulk_messages import (
    block_bulk_messages,
)
from employee.views.daily_salary.daily_salary_delete_bulk.messages.delete_bulk_messanges import (
    delete_bulk_messages,
)
from employee.views.daily_salary.daily_salary_delete_bulk.messages.messages_bulk_preview import (
    preview_items,
)
...
def daily_salary_delete_bulk(request):
    ...
    blocked_qs, deletable_qs = get_deletable_and_blocked_qs(ids, department)

    deletable_count = deletable_qs.count()
    deletable_items, deletable_tail = preview_items(deletable_qs, deletable_count)

    blocked_count = blocked_qs.count()
    blocked_items, blocked_tail = preview_items(blocked_qs, blocked_count)

    deleted_count = 0
    if deletable_qs.exists():
        deleted_count, _deleted_details = deletable_qs.delete()

    delete_bulk_messages(request, deleted_count, deletable_items, deletable_tail)

    block_bulk_messages(request, blocked_count, blocked_items, blocked_tail)
