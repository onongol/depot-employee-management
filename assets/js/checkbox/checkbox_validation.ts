...
function getEl<T extends Element = Element>(selector: string, ctx: ParentNode = document): Maybe<T> {
  return (ctx.querySelector(selector) as Maybe<T>) ?? null;
}

function showSelectionError(tableDiv: HTMLElement | null, errorId: string, errorMessage: string): void {
  if (!tableDiv) return;
...
function hideSelectionError(tableDiv: HTMLElement | null, errorId: string): void {
  if (!tableDiv) return;
...
export function setupCheckboxValidation(formId: string, checkboxName: string, tableSelector: string, errorMessage: string): void {
  const form = document.getElementById(formId) as Maybe<HTMLFormElement>;
  if (!form) return;

  const tableDiv = (form.querySelector(tableSelector) ?? document.querySelector(tableSelector)) as Maybe<HTMLElement>;
  if (!tableDiv) return;

  const errorId = `${checkboxName}-selection-error`;

  form.addEventListener('submit', (e: Event) => {
    const checked = form.querySelectorAll<HTMLInputElement>(`input[name="${checkboxName}"]:checked`);
    if (checked.length === 0) {
      e.preventDefault();
  ...
  const selectAll = tableDiv.querySelector<HTMLInputElement>('input[type="checkbox"][id^="select-all"]');
  if (selectAll) {
...
document.addEventListener('DOMContentLoaded', function () {
  document.querySelectorAll<HTMLElement>('[data-checkbox-validation="true"]').forEach(el => {
    const formId = el.getAttribute('data-form-id') || el.id || 'createForm';
    const checkboxName = el.getAttribute('data-checkbox-name') || 'employee_ids';
    const tableSelector = el.getAttribute('data-table-selector') || '.employee-table-container';
    const errorMessage = el.getAttribute('data-error-message') || '';
    if (formId) {
      setupCheckboxValidation(formId, checkboxName, tableSelector, errorMessage);
    }
  });
});