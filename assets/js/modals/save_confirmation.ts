...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const confirmBtn = document.getElementById('confirmSaveButton') as HTMLButtonElement | null;
  const updateForm = document.getElementById('updateForm') as HTMLFormElement | null;

  const isModalOpen = (): boolean => !!saveModal && !saveModal.classList.contains('hidden');
  ...
  if (confirmBtn && updateForm) {
    confirmBtn.addEventListener('click', (e: MouseEvent) => {
      ...
      if (typeof (updateForm as HTMLFormElement & { requestSubmit?: Function }).requestSubmit === 'function') {
        (updateForm as HTMLFormElement & { requestSubmit?: Function }).requestSubmit();
      } else {
        updateForm.submit();
      }
      closeSaveModal();
