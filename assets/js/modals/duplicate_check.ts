...
document.addEventListener('DOMContentLoaded', () => {
  const existingPieceworksEl = document.getElementById('existing-pieceworks') as HTMLScriptElement | null;
  const existingPieceworks: ExistingPiecework[] = (() => {
    if (!existingPieceworksEl || !existingPieceworksEl.textContent) return [];
    try {
      return JSON.parse(existingPieceworksEl.textContent) as ExistingPiecework[];
    } catch {
      return [];
    }
  })();
  ...
  const modalDiv = document.getElementById('saveDuplicateModal') as HTMLElement | null;
  const confirmBtn = document.getElementById('confirmSaveBtn') as HTMLButtonElement | null;
  const cancelBtn = document.querySelector('#saveDuplicateModal [data-modal-cancel]') as HTMLElement | null;
  ...
  form.addEventListener('submit', (e) => {
    ...
    // Collect selections
    const selectedEmployeeIds = Array.from(document.querySelectorAll<HTMLInputElement>('input[name="employee_ids"]:checked')).map(cb => cb.value);
    const selectedWorkIds = Array.from(document.querySelectorAll<HTMLInputElement>('input[name="work_ids"]:checked')).map(cb => cb.value);

    const typeWorkEl = document.getElementById('type_work') as HTMLSelectElement | HTMLInputElement | null;
    const workDateEl = document.getElementById('work_date') as HTMLInputElement | null;
    const wagonNumberInput = document.getElementById('wagon_number') as HTMLInputElement | null;
