...
document.addEventListener("DOMContentLoaded", () => {
	...
	// PRECOMPUTE: set of composite keys for fast lookup
	const makeDuplicateKey = (
		...
		`${keyEmpID}::${keyWorkID}::${keyTypeWork}::${keyWorkDate}::${keyWagonNumber ?? ""}`;

	const existingSet = new Set<string>(
		existingPieceworks.map((piecework) =>
			makeDuplicateKey(
				String(piecework.employee_id),
				String(piecework.work_id),
				piecework.type_work,
				piecework.work_date,
				normalizeWagon(piecework.wagon_number),
			),
		),
	);
	...
	form.addEventListener("submit", (event) => {
		...
		const typeWorkEl = (document.getElementById(
			DUPLICATE_ELEMENTS.elTypeWork,
		) ||
			document.getElementById(DUPLICATE_ELEMENTS.elIdTypeWork) ||
