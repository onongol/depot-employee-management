...
document.addEventListener("DOMContentLoaded", () => {
	...
	function getDropdownElements(root: HTMLElement): DropdownElements | null {
		...
		const menu = root.querySelector<HTMLElement>(DROPDOWN_SELECTORS.dropdownMenu);
	...
	function setActivePanel(menu: HTMLElement, panelName: PanelName): void {
		menu.querySelectorAll<HTMLElement>(DROPDOWN_SELECTORS.panels).forEach((panel) => {
			panel.classList.toggle(DROPDOWN_CLASSES.hidden, panel.dataset.panel !== panelName);
		});
	}
		...
		menu
			...
			.forEach((trigger) => {
				trigger.addEventListener("click", (e: MouseEvent) => {
					...
					if (!target || !Object.values(DROPDOWN_PANELS).includes(target)) return;
